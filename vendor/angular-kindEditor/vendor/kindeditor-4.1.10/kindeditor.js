...
(function(window, undefined) {
	...
	function _json(text) {
		...
		var p1 = text.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g, '@');
	...
	K.options = {
		...
		// Modified by Jason at 2018/01/09 ST
		layout: '<div class="container"><div class="toolbar"></div><div class="edit"></div><div class="statusbar"></div></div>',
		ajaxCustomHeaders: {},
		uploadResultJson: ''
		// Modified by Jason at 2018/01/09 ED
	};
	...
	_extend(KUploadButton, {
		...
		submit: function() {
			var self = this,
				iframe = self.iframe,
				form = self.form;
			var paramRegion = K('input[type="hidden"]', form);
			var randCode = '' + new Date().valueOf() + (~~(Math.random() * 9)) + (~~(Math.random() * 9)) + (~~(Math.random() * 9));
			var paramField = K('input[name="unique_code"]', form);
			if(paramField.length == 0) {
				paramField = K('<input type="hidden" name="unique_code" value="' + randCode + '" />');
				paramRegion.after(paramField);
			} else {
				paramField.value = randCode;
			}
			...
			iframe.bind('load', function() {
				...
				// JasonDong Modify 2018-02-01 ST
				// get url by ajax
				K.ajax(K.addParam(K.options.uploadResultJson, 'uniqueCode=' + randCode), function(data) {

					try {
						if(data) {
							self.options.afterUpload.call(self, data);
						} else {
							self.options.afterError.call(self, '<!doctype html><html>' + K.lang('multiimage.unknownError') + '</html>');
						}
					} catch(e) {
						self.options.afterError.call(self, '<!doctype html><html>' + K.lang('multiimage.unknownError') + '</html>');
					}
	...
	function _ajax(url, fn, method, param, dataType) {
		...
		// Modified by Jason at 2018/01/09 ST
		var token = K.options.ajaxCustomHeaders || {};
		_each(token, function(key, val) {
	...
	function _create(expr, options) {
		...
		// Modified by Jason at 2018/01/09 ST
		options.ajaxCustomHeaders = _undef(options.ajaxCustomHeaders, {});
		K.options.ajaxCustomHeaders = _undef(options.ajaxCustomHeaders, K.options.ajaxCustomHeaders);
		options.uploadResultJson = _undef(options.uploadResultJson, '');
		K.options.uploadResultJson = _undef(options.uploadResultJson, K.options.uploadResultJson);
		// Modified by Jason at 2018/01/09 ED
